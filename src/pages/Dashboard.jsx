...
//   const [queries, setQueries] = useState([]); // List of queries from queries table
//   const [subscribedTags, setSubscribedTags] = useState([]); // Subscribed tags from newsletters
//   const [sentimentData, setSentimentData] = useState(null); // Sentiment pie chart data
//   const [mostSearchedHashtag, setMostSearchedHashtag] = useState(''); // Most searched hashtag
//   const [sentimentSummary, setSentimentSummary] = useState(''); // Sentiment summary
//   const [loading, setLoading] = useState(false);
//   const [error, setError] = useState(null);
//   const [newsletterLoading, setNewsletterLoading] = useState(false); // Loading state for newsletter button

//   // Fetch queries and subscribed tags on component mount
//   useEffect(() => {
...
//         const newslettersResponse = await axios.get(`${API_URL}/get_subscribed_tags`, {
//           headers: { 'x-access-token': token },
//         });
...
//   // Fetch sentiment data, most searched hashtag, and summary when platform and query change
//   useEffect(() => {
...
//   // Filter queries based on selected platform and remove duplicates
...
//       <button
//         onClick={goToHome}
//         className="absolute top-6 right-6 px-6 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center gap-2 group"
//       >
//         <svg
...
//                 <div
//                   key={index}
//                   className="p-4 bg-purple-50 rounded-xl shadow-sm hover:bg-purple-100 hover:shadow-md transition-all duration-300 flex justify-between items-center group"
//                 >
//                   <span className="font-medium text-gray-800">{tag.tag}</span>
//                   <span className="text-sm text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
//                     {tag.platform}
//                   </span>
//                 </div>
//               ))}
