...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const queriesResponse = await axios.get(`${API_URL}/get_queries`, { headers: { 'x-access-token': token } });
        setQueries(queriesResponse.data.queries || []);

        const newslettersResponse = await axios.get(`${API_URL}/get_subscribed_tags`, { headers: { 'x-access-token': token } });
        setSubscribedTags(newslettersResponse.data.subscribed_tags || []);

        const usersResponse = await axios.get(`${API_URL}/get_users`, { headers: { 'x-access-token': token } });
  ...
  useEffect(() => {
    ...
    const fetchSentimentData = async () => {
      ...
      try {
        ...
        setError(error.response?.data.message || "An error occurred while fetching sentiment data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUnsubscribe = async () => {
    ...
    try {
      ...
      if (response.data.message) {
        ...
        setSubscribedTags(subscribedTags.filter(
          sub => !(sub.platform === selectedTag.platform && sub.tag === selectedTag.tag)
        ));
      }
    } catch (error) {
      ...
      <div className="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-2xl border border-gray-100">
        <div className="flex flex-col sm:flex-row gap-6 items-center">
          <div className="w-full sm:w-1/2">
            ...
            <select value={platform} onChange={(e) => { setPlatform(e.target.value); setQuery(''); }} className="w-full p-4 bg-gradient-to-r from-gray-50 to-gray-100 border-2 border-indigo-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-gray-700 shadow-sm hover:shadow-md transition-shadow duration-300">
          ...
          <div className="w-full sm:w-1/2">
            ...
            <div className="flex items-center gap-4">
              <select value={query} onChange={(e) => setQuery(e.target.value)} className="w-full p-4 bg-gradient-to-r from-gray-50 to-gray-100 border-2 border-purple-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-700 shadow-sm hover:shadow-md transition-shadow duration-300" disabled={!platform}>
                <option value="">Select Tag</option>
                {filteredQueries.map((queryValue, index) => (
                  <option key={index} value={queryValue}>{queryValue}</option>
                ))}
              </select>
              <button onClick={() => { setPlatform(''); setQuery(''); setSentimentData(null); setMostSearchedHashtag(''); setSentimentSummary(''); }} className="px-6 py-2 bg-gradient-to-r from-red-500 to-red-700 text-white rounded-xl hover:from-red-600 hover:to-red-800 transition-all duration-300 shadow-sm hover:shadow-md">Reset</button>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="bg-white/90 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-gray-100">
              ...
              <div className="flex flex-col lg:flex-row-reverse gap-8">
                    ...
                    <div className="relative w-full max-w-md">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="bg-white/95 rounded-full px-4 py-2 shadow-md border border-gray-200">
                          <p className="text-sm font-semibold text-gray-700">Total: {sentimentData.datasets[0].data.reduce((a, b) => a + b, 0)}</p>
                        </div>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="lg:w-2/3">
                  ...
                  {sentimentSummary ? (
                    <div className="text-gray-600 prose prose-ul:pl-5 prose-li:marker:text-blue-600 h-[450px] overflow-y-auto scrollbar scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-gray-100 scrollbar-thumb-rounded-full scrollbar-track-rounded-full p-6 bg-gradient-to-br from-gray-50 to-blue-50 rounded-lg shadow-inner border border-gray-200" dangerouslySetInnerHTML={{ __html: sentimentSummary }} />
                  ) : (
        ...
        <div className="bg-white/80 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={handleGetNewsletter} disabled={subscribedTags.length === 0 || newsletterLoading} className={`px-6 py-2 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 flex items-center gap-2 ${subscribedTags.length === 0 || newsletterLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-500 to-purple-700 text-white hover:from-purple-600 hover:to-purple-800'}`}>
              {newsletterLoading ? (
