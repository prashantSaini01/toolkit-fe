...
function CreationProgress() {
  const { data, status } = useSelector((state) => state.content);
  const [expanded, setExpanded] = useState(null);

  // If no data or status is loading with no results yet, show loading state
  if (
    status === "loading" &&
    (!data || data.every((item) => !item.results || item.results.length === 0))
  ) {
    return (
      <div className="mt-8">
  ...
  // Flatten all results from data items for display
