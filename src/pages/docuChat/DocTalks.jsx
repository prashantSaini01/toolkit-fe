...
function Lawbot() {
  ...
  const checkDocumentProcessingStatus = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/docu_chat/sessions/${currentSession}/processing_status`
      );
  ...
  const fetchSessions = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`${API_URL}/docu_chat/sessions`);
      setSessions(response.data);
    } catch (error) {
  ...
  const renameSession = async (oldSessionId, newName) => {
    ...
    try {
      const response = await axios.put(
        `${API_URL}/docu_chat/sessions/${oldSessionId}/rename`,
        {
          new_name: newName,
        },
        {
      ...
      setSessions((prevSessions) =>
        prevSessions.map((session) =>
          session.session_id === oldSessionId
            ? {
                ...session,
                agent_config: { ...session.agent_config, name: newName },
              }
            : session
        )
      );
  ...
  const selectSession = async (sessionId) => {
    ...
    try {
      const messagesResponse = await axios.get(
        `${API_URL}/docu_chat/sessions/${sessionId}/messages`
      );
      setChatHistory(messagesResponse.data);
      const pdfsResponse = await axios.get(
        `${API_URL}/docu_chat/sessions/${sessionId}/pdfs`
      );
      setUploadedPdfs(pdfsResponse.data);
      setEmbedCode("");
    } catch (error) {
  ...
  const deleteSession = async () => {
    if (currentSession) {
      ...
      try {
        ...
        await axios.delete(`${API_URL}/docu_chat/sessions/${currentSession}`);
        setSessions((prevSessions) =>
          prevSessions.filter(
            (session) => session.session_id !== currentSession
          )
        );
        setCurrentSession(null);
        setChatHistory([]);
  ...
  const fetchChatHistory = async () => {
    if (currentSession) {
      ...
      try {
        const response = await axios.get(
          `${API_URL}/docu_chat/sessions/${currentSession}/messages`
        );
        setChatHistory(response.data);
      } catch (error) {
        console.error("Failed to fetch chat history:", error);
  ...
  const fetchUploadedPdfs = async () => {
    if (currentSession) {
      ...
      try {
        const response = await axios.get(
          `${API_URL}/docu_chat/sessions/${currentSession}/pdfs`
        );
        setUploadedPdfs(response.data);
  ...
  const processDocuments = async () => {
    ...
    try {
      await axios.post(
        `${API_URL}/docu_chat/sessions/${currentSession}/process_documents`,
        {
  ...
  const generateEmbedCode = async () => {
    if (currentSession) {
      ...
      try {
        // Pass the session ID to be used for documents only
        const response = await axios.get(
          `${API_URL}/docu_chat/sessions/${currentSession}/embed?documentsOnly=true`
        );
        setEmbedCode(response.data.embed_code);
      } catch (error) {
        console.error("Failed to generate embed code:", error);
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-sm text-gray-500">
                Upload legal documents and chat with an AI assistant about their
                content.
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row gap-6">
          <div className="w-full md:w-1/3 lg:w-1/4">
            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-lg font-medium text-gray-900 mb-4">Agent Management</h2>
                  ...
                  <div className="mt-6">
                    ...
                    {embedCode && (
                      <div className="mt-4">
                        ...
                        />
                        <p className="mt-2 text-sm text-gray-500">
                          Paste this into your browser’s console to display the
                          chatbot on any website.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 h-full">
              ...
              {currentSession ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-gray-300 rounded-lg p-12">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    Please select or create a session to start chatting.
                  </p>
                  <button
                    className="mt-4 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    onClick={createNewSession}
