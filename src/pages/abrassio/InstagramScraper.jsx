...
import { scrapeInstagram, clearPosts } from "../../redux/slices/abrassio/instagramScraperSlice";
import { format } from "date-fns";

const InstagramScraper = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { posts, loading, error } = useSelector((state) => state.instagramScraper);

  const [query, setQuery] = useState("");
  const [useCache, setUseCache] = useState(true);

  const handleScrape = () => {
    dispatch(scrapeInstagram({ hashtag: query.trim(), useCache })).then((result) => {
      if (result.error && result.payload === "Session expired.") {
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download = "instagram_scraped_posts.csv";
