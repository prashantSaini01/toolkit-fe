...
import React, { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
//import logo from '../assets/Superman-Emblem-removebg-preview.ico'; // Adjust the path if needed
import logo from '../assets/abra.png'
const Header = () => {
  const navigate = useNavigate();
  const token = localStorage.getItem('token');
    ...
    <header className="bg-primary text-white p-3 shadow-custom fixed top-0 left-0 w-full z-50">
      <nav className="container mx-auto flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          <img src={logo} alt="Logo" className="h-10 w-10 md:h-10 md:w-10 rounded-full object-cover" />
          <h1 className="text-2xl md:text-3xl font-bold tracking-wide">
            Scraping Assistant
          </h1>
        </div>
